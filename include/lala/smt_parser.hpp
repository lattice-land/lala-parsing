...
class SMTParser {
 ...
 private:
  ...
  F make_variable_decl(const SV& sv) { 
    ...
    // // In RL application, we need circle constraints, x^2 + y^2 = 1.
    // // Yet, it is not yet suppoorted in other existing tools.
    // // Hence, to simply try if turbo can handle this type of problems, we introduce these circle constraints in parser directly.
    // // It is not ideal solution, it is only for testing. Once the results are good, we will change this implementation to be more general.
    // auto var_name = std::any_cast<std::string>(sv[0]);
    // std::map<std::string, std::pair<std::string, std::string>> circle_map = {
    //   {"X_4", {"X_2", "X_3"}},
    //   {"X_9", {"X_5", "X_7"}},
    //   {"X_10", {"X_6", "X_8"}},
    //   {"X_19", {"X_15", "X_17"}},
    ...
    // for (const auto& c : circle_map){
    //   if (var_name == c.first) {
    //     FSeq seq; 
    //     seq.push_back(F::make_binary(F::make_lvar(UNTYPED, LVar<allocator_type>(c.second.first)), MUL, F::make_lvar(UNTYPED, LVar<allocator_type>(c.second.first))));
    //     seq.push_back(F::make_binary(F::make_lvar(UNTYPED, LVar<allocator_type>(c.second.second)), MUL, F::make_lvar(UNTYPED, LVar<allocator_type>(c.second.second))));
